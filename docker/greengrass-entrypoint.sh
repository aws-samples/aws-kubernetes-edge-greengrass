...
OPTIONS="-Droot=${GGC_ROOT_PATH} -Dlog.store=FILE -Dlogging.outputDirectory=/logs -Dlog.level=${LOG_LEVEL} -jar ${INIT_JAR_PATH}/lib/Greengrass.jar --provision ${PROVISION} --deploy-dev-tools ${DEPLOY_DEV_TOOLS} --start true"

echo "Using the following option: $OPTIONS"
